...
In order to start building ISO file go to this "page":https://github.com/dellcloudedge/crowbar/wiki/Build-Crowbar.ISO describing how to start this process and then add barclamp-cassandra sub module by running the commands:
...
After that you need to build ISO file containing barclamp-cassandra and all dependencies needed for it. Crowbar will automatically download packages and put them into ISO file.
...
DELL Crowbar needs one machine for an admin node and a several machines for working nodes.
...
The "test_framework":https://github.com/dellcloudedge/crowbar/tree/master/test_framework which is a part of Crowbar is used to deploy nodes on KVM. Run the command below to deploy Crowbar on KVM machines:
